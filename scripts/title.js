...
$(document).ready(function() {
    ...
    let update = () => {
        ...
        let time = performance.now() * 0.00001 * morphSpeedValue * Math.pow(processingValue, 3),
            spikes = spikesValue * processingValue;

        for(let i = 0; i < sphere.geometry.vertices.length; i++) {
            let p = sphere.geometry.vertices[i];
            p.normalize().multiplyScalar((1 + 0.3 * simplex.noise3D(p.x * spikes, p.y * spikes, p.z * spikes + time))*sphereRadiusValue);
        }
